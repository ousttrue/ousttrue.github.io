...
import { PAGES, POSTS, DIRS, TAGS, COPYDATA } from "./pages.ts";
...
export async function generate(dist: string) {
  // 1. index.html を読み込む
  const template_src = fs.readFileSync("index.html", "utf-8");
  ...
  for (const [url, _] of Object.entries(DIRS)) {
    if (!(url in POSTS)) {
      await prerenderAndWrite(template_src, url, dist);
    }
  }
  ...
  for (const src of Object.keys(COPYDATA)) {
    const root = "/src/pages";
    const dst = path.join(dist, src.substring(root.length));
    console.log("copy ...:", dst);
    fs.copyFileSync(path.join(".", src), dst);
  }
}
