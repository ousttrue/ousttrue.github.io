...
class Repo:
    def __init__(self, item) -> None:
        ...
        self.pushed_at = datetime.datetime.strptime(item["pushed_at"], DATE_FORMAT)
...
#
@task
def dl_zenn(c: Context, user: str):
    """
    download zenn articles
    """

    # https://zenn.dev/niiharamegumu/articles/8f00cfdf9753d1
    conn = http.client.HTTPSConnection("zenn.dev", 443)
    conn.request("GET", f"/api/articles?username={user}&order=latest")
    res = conn.getresponse()
    print(res.status, res.reason)
    path = DOWNLOAD_DIR / "zenn.json"
    data = res.read()
    path.write_bytes(data)
...
class Zenn:
    def __init__(self, item):
        self.title = escape_title(item["title"])
        self.url = f'https://zenn.dev{item["path"]}'
        self.tags = ["zenn"]
        self.created_at = datetime.datetime.fromisoformat(item["published_at"])
        self.updated_at = datetime.datetime.fromisoformat(item["body_updated_at"])
...
def zenn(c: Context):
    ...
    generate Zenn
    """
    items = json.loads((DOWNLOAD_DIR / "zenn.json").read_bytes())["articles"]
    dir = HERE / "content/zenn"
    files = {f: True for f in dir.glob("*.md")}
    dir.mkdir(exist_ok=True, parents=True)
    for item in items:
        zenn = Zenn(item)
        ...
        body = zenn.format_md().encode("utf-8")
        if path.exists() and path.read_bytes() == body:
            continue
